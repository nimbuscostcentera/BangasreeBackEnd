...
class SuperUserController {
  ...
  async passbookreturn(req, res, next) {
    try {
      const getUserResponse = await PassBookAssignService.ReturnPassBookToAgent(req, res, next);
      next();
      return getUserResponse;
    } catch (err) {
      console.log(err);
  ...
  async passbookreturncust(req, res, next) {
    try {
      const getUserResponse = await PassBookAssignService.ReturnPassBookToCustomer(req, res, next);
      next();
      return getUserResponse;
    } catch (err) {
      console.log(err);
  ...
  async maturitycertificateshow(req, res, next) {
    try {
      const getUserResponse = await MaturityStatusService.MaturityCertificateShow(
        req,
        res,
        next
      );
