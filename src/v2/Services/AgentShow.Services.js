...
class ShowAgntService {
  async AgentShow(req, res, next) {
    try {
      ...
      sql =
        sql +
        "select a.*,br.BranchId,br.branchname,br.branchcode,ar.areaname from agentmasters as a,areamasters as ar,usermasters as u,branchmasters as br where u.uuid=a.uuid and u.branchid=br.branchid and br.areaid=ar.areaid";
      if (
        (req.body.Status != "" &&
