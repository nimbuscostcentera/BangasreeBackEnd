      ...
      const Custsw = await sq
        ...
        .then(async () => {
          ...
          if (ID == "" || ID == undefined || ID == null) {
            sql =
              "SELECT SM.SCHEMETITLE,SR.EMI,SR.frequency,SR.CustomerAccNo,SR.ID,SM.Regfees,SM.Duration,SM.BONUS,SR.PassBookNo FROM schemeregisters AS SR ,schememasters AS SM WHERE SM.SUUid=SR.SUUid and  SM.Status=1 and SR.MaturityStatus = 1  ";
            qt.uuid = CustUUid;
          } else {
            sql =
              "SELECT SM.SCHEMETITLE,SR.EMI,SR.frequency,SR.CustomerAccNo,SR.ID,SM.Regfees,SM.Duration,SM.BONUS,SR.PassBookNo FROM schemeregisters AS SR ,schememasters AS SM WHERE SM.SUUid=SR.SUUid and SM.Status=1 and SR.MaturityStatus = 1 and SR.ID=:ID   ";
