    ...
    try {
      ...
      if (Utype == 1) {
        if (SuperUserType == 1) {
          ...
          sql = sql+'(SELECT COUNT(Id) FROM schemeregisters JOIN usermasters on  schemeregisters.UUid = usermasters.UUid join customermasters on schemeregisters.UUid = customermasters.UUid  WHERE MaturityStatus=3 and schemeregisters.updatedAt  =:today   '
...
console.log("in card");
       ...
       (sql = `SELECT
           ...
           (SELECT COALESCE(SUM(et.CollectedAmt), 0) FROM emitrans as et,agentmasters as am where et.AgentUUid=am.UUid and et.PaymentType=2 and  am.AgentCode= '${agentCode}' and et.CollDate =:today)  AS TotalCollection`),
         (qt.today = today);
