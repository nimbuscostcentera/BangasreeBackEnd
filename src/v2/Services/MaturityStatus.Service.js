...
const {
  MaturityCertificateMaster,
} = require("../Model/MaturityCertificateMaster.Model");
...
class MaturityStatusService {
  async MaturityStatus(req, res, next) {
    try {
      ...
      if (
        req.body.withBonus != "" &&
        req.body.withBonus != null &&
        req.body.withBonus != "undefined"
      ) {
      ...
      if (UUid?.length != 0) {
            ...
            .then(async (cal) => {
              if (cal[0].MaturityStatus == 0) {
                ...
                if (cal[0].BonusStatus == 1) {
                  matrureamt =
                    cal[0].collection +
                    (cal[0].collection * cal[0].BONUS) / 100;
                } else {
        ...
        if (flag == 1 && Status != 1) {
          ...
          return res
            ...
            .json({
              response:
                "Maturity Status Inactive.You Can Not Mature Or Premature This Acount!!",
            });
        } else {
          Custsw = await sq
            .query(sql, qt)
            .then(async (res2) => {
              ...
              if (res2[1] === 0) {
                ...
                return res
                  ...
                  .json({
                    response:
                      "Alreday Matured .You Can Not Change Maturity Status!!",
                  });
              } else {
                await schemeregisterhistory
                  ...
                  .then(async (rst) => {
                    ...
                    return res
                      ...
                      .json({
                        errmsg: false,
                        response: "Maturity Status Updated successfully",
                      });
                  })
                  .catch((err) => {
                    console.log(err);
  ...
  async MaturityCertificate(req, res, next) {
    try {
      ...
      await sq
        .query(
          "SELECT sr.CustomerAccNo,sr.MaturityStatus,sr.MaturityComment,sum(et.CollectedAmt) as TotalDepositedAmount,count(CollectionId) as NoOfInstallments,sr.RedeemAmt,sr.EndDate as MaturityDate,sr.UUid FROM `schemeregisters` as sr,emitrans as et WHERE sr.ID = et.SchemeRegId and sr.id=:SchemeRegId  and sr.CompanyCode=:CompanyCode",
          {
            replacements: {
              SchemeRegId: SchemeRegId,
              CompanyCode: CompanyCode,
          ...
          await MaturityCertificateMaster.create({
            ...
            .then(async (rst) => {
              ...
              return res
                ...
                .json({
                  errmsg: false,
                  response: "Certificare Generated Sucessfully",
                });
            })
            .catch((err) => {
              console.log(err);
              return res
                ...
                .json({
                  errmsg: false,
                  response: "Certificare Generation  failed",
                });
            });
        });
      return Custsw;
