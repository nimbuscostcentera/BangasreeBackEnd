...
class CollectionViewService {
  ...
  async CustomerCollectionShow(req, res, next) {
    try {
      ...
      const { CompanyCode, MaturityStatus, BonusStatus, AgentCode, TimeToMature, CustomerID, Due, CollDate, SchemeRegId, LoggerBranchId, SuperUserType,Maturitydate } = req.body;
      const date = new Date();
      const repobj = {};
...
WHERE 1=1      `;
      ...
      if (Maturitydate) {
        sql += " AND et.updatedAt = :Maturitydate";
        repobj.Maturitydate = Maturitydate;
      }
