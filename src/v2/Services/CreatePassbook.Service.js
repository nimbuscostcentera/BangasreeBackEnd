...
class PassbookAddService {
  async CreatePassbook(req, res, next) {
    ...
    try {
      ...
      var ID;
      const { NoOfBooks, CompanyCode, BranchId } = req.body;
        ...
        await BranchMasters.findAll({
          ...
          .then(async (res3) => {
            ...
            for (i = 1; i <= NoOfBooks; i++) {
              ...
              passbookno =
                "BGPL-PB-" + res3[0].dataValues.BranchCode + "-" + srl;
  ...
  async TransferPassbook(req, res, next) {
    ...
    try {
      ...
      console.log(req.body, "try start");
      var ID;
      const { data, CompanyCode, BranchId } = req.body;
      var len;
      len = data.length;
      console.log(len);
      const passbookCreationPromises = [];
