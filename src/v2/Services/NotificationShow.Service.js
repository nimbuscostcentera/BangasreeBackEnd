...
class NotificationShowService {
  async NotificationShow(req, res, next) {
    try {
      ...
      await sq
        .query(
          " SELECT Nd.*,um.UserName as Receiver FROM notificationdetails as Nd,notificationheaders as Nm,usermasters as um WHERE Nd.`TicketId`=Nm.`TicketId` and Nd.ToUser=:UserID AND Nd.toUser=um.UserID ORDER by Nd.createdAt",
          {
