...
class ShowLeadService {
  async LeadShow(req, res, next) {
    try {
      var obj = {};
      var sql =
        "SELECT pc.CustomerID,pc.EmailId,pc.PhoneNumber,pc.CustomerName,pc.Address ,pc.FollowUpDate,pc.SuperUserID,pc.AgentCode,pc.BranchId,pc.Sex,bm.BranchCode,bm.BranchName,arm.AreaName,am.Name as Name,arm.AreaId ,bm.BranchCode,bm.BranchName FROM proabablecustomers as pc inner join  branchmasters as bm  on pc.BranchId=bm.BranchId inner join agentmasters as am on pc.AgentCode=am.AgentCode inner join areamasters as arm on arm.AreaID=pc.AreaId";
      var qt = {};
