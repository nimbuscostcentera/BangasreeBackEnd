  ...
  async AgentApprove(req, res, next) {
    ...
    try {
      ...
      if (UUid != undefined && UUid != "" && UUid != [] && UUid.length != 0) {
        ...
        Agntsw = await sq
          .query(
            "Update agentmasters set Status=:st,password=:hashPassword where AgentID in(:id) ",
            {
              replacements: {
                st: Status,
                id: UUid,
            ...
            await sq
              ...
              .then(async (res3) => {
                ...
                url =
                  ...
                  "&mobile=+91" +
                  PhoneNumber +
                  "&message=Welcome to Bangasree Jewellers. Your agent application is approved and your portal is ready to access. Login to www.bangasreejewellers.in to check your account. Your login details are below: Login ID: " +
                  PhoneNumber +
                  " Password: " +
                  password +
                  " For any query, please call 8585023758 or visit your nearest branch/office. Regards, Bangasree Jewellers&senderid=BJSWRN&accusage=1&entityid=1201170685649952029&tempid=1207171083989155584";
                console.log(url);
                http
                  .get(url, (response) => {
                    let data = "";
